...
public interface InfoMapper {
    ...
    @Select("select * from meeting as a left join (select meeting_idx, image_url " +
            "from meeting_image group by meeting_idx) as b on a.idx = b.meeting_idx left join (select meeting_idx, count(*) as scrap_cnt " +
            "from scrap group by meeting_idx) as c on b.meeting_idx = c.meeting_idx order by scrap_cnt desc") // 인기순
